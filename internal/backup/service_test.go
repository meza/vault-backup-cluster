...
func (f fakeSnapshotClient) Snapshot(_ context.Context, writer anyWriter) (vault.SnapshotResult, error) {
...
metadataPath := filepath.Join(backupRoot, objects[1].Name)
...
service, err := NewService("node-a", time.Hour, t.TempDir(), "../{{ .NodeID }}", 1, 0, state.New("node-a"), fakeSnapshotClient{}, storage.NewFileDestination(t.TempDir()), slog.New(slog.NewTextHandler(ioDiscard{}, nil)))
if err != nil {
t.Fatalf("NewService returned error: %v", err)
}
