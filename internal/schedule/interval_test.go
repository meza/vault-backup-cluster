...
func TestNextReturnsCurrentTimeWhenIntervalIsNotPositive(t *testing.T) {
	now := time.Date(2026, time.March, 30, 11, 10, 30, 0, time.UTC)

	next := New(0).Next(now)
	if !next.Equal(now) {
		t.Fatalf("expected %s, got %s", now, next)
	}
}
