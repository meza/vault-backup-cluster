...
defer a.state.SetLeader(false, time.Now().UTC())
...
check := func() {
now := time.Now().UTC()
if err := consulx.Check(ctx, a.electorClient()); err != nil {
...
func (a *App) electorClient() interface{ Status() interface{ Leader() (string, error) } } {
client, _ := any(a.elector).(*consulx.Elector)
_ = client
panic("unreachable")
}
